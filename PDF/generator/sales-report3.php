...
$qry = "SELECT 
		tbl_sale.sale_id,tbl_sale.sale_date,tbl_user.first_name AS userfname,tbl_user.last_name AS usersname,
		tbl_customer.first_name AS customerfname, tbl_customer.masm_id,tbl_customer.last_name AS customersname,
		(SELECT count(*) FROM tbl_sale_drug WHERE tbl_sale_drug.sale_id=tbl_sale.sale_id) AS items,
		tbl_sale.amount_total,tbl_sale.discount_award
		FROM tbl_sale,tbl_user,tbl_customer
		WHERE tbl_sale.user_id=tbl_user.user_id AND tbl_sale.customer_id=tbl_customer.customer_id AND YEAR(tbl_sale.sale_date)='$year' AND MONTH(tbl_sale.sale_date)='$month' ORDER BY tbl_sale.sale_date DESC;";
$res = $db->query($qry);
...
$qry = "SELECT sum(discount_award) AS discount, sum(amount_total) AS total FROM tbl_sale WHERE YEAR(sale_date)='$year' AND MONTH(sale_date)='$month'";
...
// column titles
$header = array('SALE DATE', 'CUSTOMER NAME', 'MASM ID','TOTAL','MASM AWARD');
...
//Close and output PDF document
$pdf->Output("SALES_REPORT_".$dmonth."_".$year.".pdf", 'D');
