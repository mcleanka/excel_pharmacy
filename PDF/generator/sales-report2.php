...
$qry = "SELECT tbl_sale.sale_id, tbl_sale.sale_date, tbl_user.first_name AS userfname, tbl_user.last_name AS usersname,
		...
		tbl_sale.amount_total, tbl_sale.sale_type FROM tbl_sale, tbl_user, tbl_customer WHERE tbl_sale.user_id = tbl_user.user_id AND tbl_sale.customer_id = tbl_customer.customer_id AND YEAR(tbl_sale.sale_date) = '$year' ORDER BY tbl_sale.sale_date DESC;";
$res = $db->query($qry);
...
$qry = "SELECT sale_type, sum(amount_total) AS total FROM tbl_sale WHERE YEAR(sale_date) = '$year'";
...
//Close and output PDF document
$pdf->Output("SALES_REPORT_".$year.".pdf", 'I');
