...
$qry = "SELECT tbl_sale.sale_id, tbl_sale.sale_date, tbl_user.first_name AS userfname, tbl_user.last_name AS usersname, tbl_customer.c_fname AS customerfname, tbl_customer.c_lname AS customersname, (SELECT count(*) FROM tbl_sale_drug WHERE tbl_sale_drug.sale_id = tbl_sale.sale_id) AS items,tbl_sale.amount_total, tbl_sale.sale_type FROM tbl_sale, tbl_user, tbl_customer WHERE tbl_sale.sale_type = '$sType' AND tbl_sale.user_id = tbl_user.user_id AND tbl_sale.customer_id = tbl_customer.customer_id AND tbl_customer.customer_id = $id AND YEAR(tbl_sale.sale_date) = '$year' AND MONTH(tbl_sale.sale_date) = '$month' ORDER BY tbl_sale.sale_date DESC;";
...
while ($row = mysqli_fetch_assoc($res)){
	...
	$items = $row['items'];
	$array = [$date, $sale_id, $items ,$amount_total];
	array_push($data_array, $array);
}

$qry = "SELECT sale_type, sum(amount_total) AS total FROM tbl_user, tbl_sale WHERE tbl_sale.user_id = tbl_user.user_id AND tbl_sale.customer_id = $id AND tbl_sale.sale_type = '$sType' AND YEAR(sale_date) = '$year' AND MONTH(sale_date)='$month'";
...
if($saleType == 'Credit' || $saleType == 'credit' || $saleType == 'CREDIT'){
	$title = 'Invoice';	
}else{
...
// column titles
$header = array('SALE DATE', 'SALE ID', 'NO PRODUCTS','TOTAL');
